<template>
  <div>
    ...
    <div class="f_dl f_dl_a total" id="box">
      <span>县级渠道总数：{{data.totalCount}}</span>
    </div>
    <!--   登录框 -->
    <div class="f_dl f_dl_a"  v-for="item of data.data">
      <ul class="zl_list">
        ...
        <li class="clearfix">
          归属于
          <span>{{item.agentProvince}} {{item.agentCity}}</span>
        </li>
        <li class="clearfix">
          渠道的区域
          <span>{{item.agentArea}}</span>
        </li>
        <li class="clearfix">
          上级
          <span>{{item.ext}}</span>
...
<script>
  ...
  export default {
    ...
    mounted () {
      let _this = this
      REST.getAgencyList({ id: _this.$route.params.id, type: 3 }).then(function (res) {
        _this.data = res
      })
    },
    methods: {
